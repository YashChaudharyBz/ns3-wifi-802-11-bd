...
// EDIT: include bd-tag.h
#include "ns3/bd-tag.h"
...
FrameExchangeManager::StartTransmission(Ptr<Txop> dcf, uint16_t allowedWidth)
{
    ...
    // EDIT: set 802.11bd repetitions if available
    Ptr<const Packet> pkt = mpdu->GetPacket();
    BdTag tag;
    if(pkt->PeekPacketTag(tag)) {
        txParams.m_txVector.SetRep(tag.GetRep());
    }
